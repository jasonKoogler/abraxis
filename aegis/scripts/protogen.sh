...
PROTO_DIR="${ROOT_DIR}/api/grpc/aegispb"
...
# Function to clean output directory (only removes generated .go files, preserves .proto sources)
clean_output_dir() {
    echo "Cleaning output directory..."
    if [ -d "$OUTPUT_DIR" ]; then
        find "${OUTPUT_DIR}" -name "*.pb.go" -type f -delete
    else
        mkdir -p "$OUTPUT_DIR"
    fi
}
