...
func LoadConfig() (*Config, error) {
	...
	if !isValidSessionManager(sessionManager) {
		return nil, fmt.Errorf("invalid session manager: %s", sessionManager)
	}

	useCustomJWT, err := strconv.ParseBool(os.Getenv("USE_CUSTOM_JWT"))
	if err != nil {
		return nil, fmt.Errorf("error parsing USE_CUSTOM_JWT: %w", err)
	}
	...
	tokenRotationInterval, err := time.ParseDuration(os.Getenv("TOKEN_ROTATION_INTERVAL"))
	if err != nil {
		return nil, fmt.Errorf("error parsing TOKEN_ROTATION_INTERVAL: %w", err)
	}
	...
	// Facebook
	if facebookKey := os.Getenv("FACEBOOK_KEY"); facebookKey != "" {
		providers = append(providers, Oauth2ProviderConfig{
			Name:         "facebook",
			ClientID:     facebookKey,
			ClientSecret: os.Getenv("FACEBOOK_SECRET"),
			RedirectURL:  os.Getenv("FACEBOOK_CALLBACK_URL"),
			Scopes:       strings.Split(os.Getenv("FACEBOOK_SCOPES"), ","),
		})
	}

	// Twitter
	if twitterKey := os.Getenv("TWITTER_KEY"); twitterKey != "" {
		providers = append(providers, Oauth2ProviderConfig{
			Name:         "twitter",
			ClientID:     twitterKey,
			ClientSecret: os.Getenv("TWITTER_SECRET"),
			RedirectURL:  os.Getenv("TWITTER_CALLBACK_URL"),
			Scopes:       strings.Split(os.Getenv("TWITTER_SCOPES"), ","),
		})
	}
	...
	cfg := &Config{
		...
		Auth: AuthConfig{
			AuthN: AuthNConfig{
				...
				OAuthConfig:            OAuthConfig{VerifierStorage: os.Getenv("OAUTH_VERIFIER_STORAGE"), Providers: providers},
			},
			AuthZ: AuthZConfig{
...
func validateConfig(config *Config) error {
	...
	// JWTSecret is no longer required for signing (Ed25519 KeyManager handles key material).
	// The field is retained in config for backward compatibility.
	if config.Auth.AuthN.OAuthConfig.VerifierStorage == "" {
		return fmt.Errorf("oauth verifier storage is required")
	}
	if len(config.Auth.AuthN.OAuthConfig.Providers) == 0 {
		return fmt.Errorf("at least one OAuth provider is required")
	}

	// Validate OAuth providers
	for _, provider := range config.Auth.AuthN.OAuthConfig.Providers {
		if provider.Name == "" {
			return fmt.Errorf("provider name is required")
		}
		if provider.ClientID == "" || provider.ClientSecret == "" || provider.RedirectURL == "" {
			return fmt.Errorf("incomplete configuration for provider %s", provider.Name)
		}
		if len(provider.Scopes) == 0 {
			return fmt.Errorf("scopes are required for provider %s", provider.Name)
		}
