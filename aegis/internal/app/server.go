...
// WithAuthService configures the authentication service
func WithAuthService(auth *authService.AuthManager) ServerOption {
	return func(s *Server) error {
		s.authService = auth
		// Instantiate the auth middleware.
		s.authMiddleware = adapterHTTP.NewAuthMiddleware(auth)
		return nil
	}
}
...
// Use this for endpoints such as /auth/login and /auth/register.
func (s *Server) RegisterPublicHandler(path string, handler http.Handler) {
	s.router.Mount(path, handler)
}

// RegisterProtectedHandler mounts a handler at the given path wrapped with auth and authorization middleware.
// Use this for endpoints such as /auth/logout, /auth/refresh, or any endpoint requiring an authenticated session.
func (s *Server) RegisterProtectedHandler(path string, handler http.Handler) {
	// Create a middleware chain with explicit ordering
	middlewareChain := func(h http.Handler) http.Handler {
		return s.authMiddleware.Authenticate(
			s.authMiddleware.Authorize("user")(h),
		)
	}

	// Apply the middleware chain
	protectedHandler := middlewareChain(handler)
