...
// RateLimitAlgorithm defines the algorithm used for rate limiting
type RateLimitAlgorithm string

const (
	// AlgorithmFixed applies a fixed rate limit
	AlgorithmFixed RateLimitAlgorithm = "fixed"
	// AlgorithmAdaptive adjusts rate limits based on load
	AlgorithmAdaptive RateLimitAlgorithm = "adaptive"
	// AlgorithmTokenBucket uses a token bucket algorithm
	AlgorithmTokenBucket RateLimitAlgorithm = "token_bucket"
	// AlgorithmLeakyBucket uses a leaky bucket algorithm
...
type RateLimitConfig struct {
	...
	Strategy          RateLimitAlgorithm `json:"strategy"`
...
// RateLimitService handles rate limiting for the API Gateway
...
func (s *RateLimitService) CreateRateLimit(ctx context.Context, params *CreateRateLimitParams, actorID uuid.UUID, actorType string) (*RateLimitConfig, error) {
	...
	if params.Strategy == "" {
		params.Strategy = AlgorithmFixed
	}

	// Validate adaptive parameters if using adaptive strategy
	if params.Strategy == AlgorithmAdaptive {
	...
	s.auditService.LogEvent(ctx, &domain.AuditLogReq{
		EventType:    "ratelimit.created",
		ActorType:    actorType,
...
func (s *RateLimitService) UpdateRateLimit(ctx context.Context, id uuid.UUID, params *UpdateRateLimitParams, actorID uuid.UUID, actorType string) (*RateLimitConfig, error) {
	...
	if config.Strategy == AlgorithmAdaptive {
...
func (s *RateLimitService) updateAdaptiveLoad(configID uuid.UUID, delta float64) {
	...
	if !exists || config.Strategy != AlgorithmAdaptive {
		return
	}
