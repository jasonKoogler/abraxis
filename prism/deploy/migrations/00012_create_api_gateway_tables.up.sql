...
-- Important indexes for API key lookup and filtering
...
COMMENT ON COLUMN api_keys.key_prefix IS 'First 8-16 characters of the API key for display and lookup (format: ak_XXXXXXXX)';
COMMENT ON COLUMN api_keys.key_hash IS 'SHA-256 hash of the complete API key for secure storage and verification';
COMMENT ON COLUMN api_keys.scopes IS 'Permission scopes this API key has access to (format: resource:action, e.g. users:read)';
COMMENT ON COLUMN api_keys.expires_at IS 'When this API key will expire and become invalid';
COMMENT ON COLUMN api_keys.is_active IS 'Whether this API key is currently active and can be used for authentication';
